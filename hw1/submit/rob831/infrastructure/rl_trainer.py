...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(self, itr, load_initial_expertdata, collect_policy, batch_size):
        ...
        # (1) First BC iteration: use provided expert data instead of collecting rollouts
        if itr == 0 and load_initial_expertdata is not None:
            with open(load_initial_expertdata, 'rb') as f:
                loaded_paths = pickle.load(f)
            # optional sanity print
            nsteps = sum(len(p["reward"]) for p in loaded_paths)
            print(f"Loaded expert dataset: {len(loaded_paths)} trajs, {nsteps} steps")
            return loaded_paths, 0, None

        # (2) Otherwise (e.g., DAgger), collect with current policy
