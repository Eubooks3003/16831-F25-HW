...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(
        ...
        use_parallel = bool(self.params.get('parallel_collect', False))
        num_workers = int(self.params.get('num_workers', 4))
        use_parallel = use_parallel and (num_workers > 1)

        print("\nCollecting data to be used for training..."
              + (" [parallel x{}]".format(num_workers) if use_parallel else ""))
        ...
        if not use_parallel:
            ...
            for w in range(num_workers):
                t = threading.Thread(
                    ...
                    kwargs=dict(
                        timesteps_target=per_worker,
                        collect_policy=collect_policy,
                        out_q=out_q,
                        seed_offset=1000 + 97 * w,  # decorrelate env RNGs
                    ),
                    daemon=True,
                )
                threads.append(t)
            ...
            agg_paths, envsteps_this_batch = [], 0
            for _ in range(num_workers):
                worker_paths, worker_n = out_q.get()
                agg_paths.extend(worker_paths)
                envsteps_this_batch += worker_n
