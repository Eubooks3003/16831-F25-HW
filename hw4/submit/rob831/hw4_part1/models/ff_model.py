...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        # predicted change in obs (normalized delta)
        concatenated_input = torch.cat([obs_normalized, acs_normalized], dim=1)

        # network outputs normalized delta; unnormalize to get delta, then add to s_t
        delta_pred_normalized = self.delta_network(concatenated_input)          # TODO(Q1)
        delta_pred = unnormalize(delta_pred_normalized, delta_mean, delta_std)  # TODO(Q1)
        next_obs_pred = obs_unnormalized + delta_pred                           # TODO(Q1)
        return next_obs_pred, delta_pred_normalized

    def get_prediction(self, obs, acs, data_statistics):
        # get the predicted next-states (s_{t+1}) as a numpy array
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        # compute normalized target: Normalize(s_{t+1} - s_t)
        ...
        # forward pass to get predicted normalized delta
